...
class Runtime:
    ...
    def prepare(self,settings,plan,provider):
        ...
        apps=[a for a in apps if a.get('domain')=='hermesaki.'+settings['domain']]
        ...
        c=Config(mode='production',domain=settings['domain'],mail_host='mail.'+settings['domain'],management_url='https://mail.'+settings['domain']+':443',
                 public_url='https://hermesaki.'+settings['domain'],access_team=plan['options']['access_team'],access_aud=apps[0]['aud'])
