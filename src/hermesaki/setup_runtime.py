...
class Runtime:
    ...
    def prepare(self,settings,plan,provider):
        ...
        c=Config(mode='production',domain=settings['domain'],mail_host='mail.'+settings['domain'],management_url='https://mail.'+settings['domain']+':443',
                 public_url='https://'+web_hosts(settings)[0],webmail_url='https://'+web_hosts(settings)[1],access_team=plan['options']['access_team'],access_aud=apps[0]['aud'],admin_emails=[settings['owner_email']])
