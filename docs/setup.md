...
`POST /v1/setup/plan` refreshes the plan, and `GET /v1/setup` returns the last plan. Changing domain configuration discards the stale plan. Compatible existing DNS, owner-only Access policies and tunnel routes are reused. Differing settings produce explicit conflicts; planning never overwrites them. Before provisioning the reviewed resources, setup verifies DNS, Tunnel and Access writes using an isolated TXT record, a disconnected deny-all tunnel and an owner-only Access application with no DNS route. These probes are shown in the plan and deleted before provisioning continues. Missing permissions or incomplete cleanup block provisioning; an uncertain write stops for reconciliation. To authorize exactly the saved plan, call `POST /v1/setup/apply` with `{"confirm_plan_id":"<saved-plan-id>"}` or confirm the displayed plan in the form. The service rechecks remote state before the first mutation, creates and verifies Access gates before routes/DNS, preserves unrelated resources, and checkpoints acknowledged operations. A changed plan requires review. An operation with an unknown outcome stops for review rather than risking duplicate writes. Configuration and plan replacement are locked once provisioning starts. A new provider credential can still be validated and saved without changing the approved plan or completed steps. This first plan applies web protection. A separate service plan below deploys the connector and mail.
...
External mail, remote-agent and network verification now have owner-operated API and form flows described below. Their individual live checks have passed on the adopted deployment. Two complete independent fresh setup journeys are still required; gate tests with explicit verifier fixtures do not establish that acceptance.
