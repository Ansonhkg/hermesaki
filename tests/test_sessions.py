...
from hermesaki import sessions
class SessionTests(unittest.TestCase):
 ...
 def call(self,method='GET',cookie='',bearer='',origin=None):
 ...
 def login(self):
  status,h,d=self.call('POST',bearer=self.token['token'],origin=self.config.public_url)
 ...
 def test_operator_revocation_and_encryption(self):
  c=self.login()
  with self.store.db() as db:
   ...
   db.execute('UPDATE tokens SET revoked=1 WHERE id=?',(self.token['id'],))
  self.assertEqual(self.call(cookie=c)[0],'401 Unauthorized')
