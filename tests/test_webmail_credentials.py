...
class WebmailCredentialsTests(SessionTests):
 def test_only_operator_can_reveal_and_audit_has_no_password(self):
  ...
  def request(token='',cookie=''):
   env={'REQUEST_METHOD':'GET','PATH_INFO':'/v1/inboxes/box/webmail-credentials','wsgi.input':io.BytesIO(b''),'HTTP_AUTHORIZATION':'Bearer '+token if token else '', 'HTTP_COOKIE':cookie}
  ...
  response,data=request(cookie=self.login());self.assertEqual(response[0],'200 OK');self.assertEqual(data['password'],password);self.assertEqual(response[1]['Cache-Control'],'no-store')
  with self.store.db() as db:
