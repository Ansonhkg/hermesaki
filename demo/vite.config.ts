...
export default defineConfig({esbuild:{jsx:'automatic'},root:resolve(root,'demo'),server:{hmr:false,watch:{ignored:['**/.runtime/**']},host:'127.0.0.1',port:19195,strictPort:true,fs:{allow:[root]}},plugins:[{name:'hermesaki-fixtures',configureServer(server){let api=mockApi();server.middlewares.use(async(req,res,next)=>{
 ...
 if(path==='/v1/auth'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify({mode:'password',configured:true}));return;}
 if(path==='/v1/session'){let body='';for await(const chunk of req)body+=chunk;let data:any={};try{data=body?JSON.parse(body):{}}catch{}const logged=req.headers.cookie?.includes('hermesaki_demo_session=1');const ok=req.method==='POST'?data.username==='admin'&&data.password==='local-preview-only':logged;res.statusCode=ok?200:401;if(ok&&req.method!=='GET')res.setHeader('Set-Cookie',`hermesaki_demo_session=${req.method==='POST'?'1':''}; Path=/; HttpOnly; SameSite=Strict`);res.setHeader('Content-Type','application/json');res.end(JSON.stringify(ok?{authenticated:req.method!=='DELETE'}:{error:'invalid_credentials'}));return;}
