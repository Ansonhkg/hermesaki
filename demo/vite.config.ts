...
export default defineConfig({esbuild:{jsx:'automatic'},root:resolve(root,'demo'),server:{host:'127.0.0.1',port:19195,strictPort:true,fs:{allow:[root]}},plugins:[{name:'hermesaki-fixtures',configureServer(server){let api=mockApi();server.middlewares.use(async(req,res,next)=>{
 ...
 if(ext==='html')body=Buffer.from(body.toString().replace('</body>','<script type="module" src="/bridge.ts"></script></body>').replace('<body>','<body><div style="background:#fff0c2;color:#513c00;padding:12px;font:14px system-ui">LOCAL WALKTHROUGH · Fictional data. Mail, DNS and Cloudflare responses are simulated.</div>'));
 res.end(body);return;
 }next();
 });}}]});
