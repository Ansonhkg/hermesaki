...
fingerprint = hashlib.sha256((root / "components.lock.json").read_bytes() + (root / "patches/mcpcube-compatibility.patch").read_bytes() + (root / "integrations/mcpcube/composer.lock").read_bytes()).hexdigest()
marker = cache / ".hermesaki-prepared"
if cache.exists():
    if marker.exists() and marker.read_text() == fingerprint:
        print("Pinned source already prepared.")
        raise SystemExit(0)
    raise SystemExit("Source cache differs or is incomplete. Remove .cache/mcpcube explicitly before preparing again.")
cache.parent.mkdir(exist_ok=True)
subprocess.run(["gh", "repo", "clone", component["repository"], str(cache)], check=True)
subprocess.run(["git", "-C", str(cache), "checkout", "--detach", component["revision"]], check=True)
subprocess.run(["git", "-C", str(cache), "apply", str(root / "patches/mcpcube-compatibility.patch")], check=True)
